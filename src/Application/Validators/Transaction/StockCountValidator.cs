...
{
    public class StockCountValidator : IStockCountValidator
    {

        private BLTransaction _transaction;
        public UserMessageManager UserMessages { get; set; }

        public StockCountValidator(BLTransaction transaction)
        {
        ...
        public bool CanAddItemToGrid(decimal? TransactionQuantiy = null)
        {
            ...
            {
                if (_transaction.SelectedLineItem.TransactionQuantity> TransactionQuantiy.Value)
                {
                    UserMessages.AddErrorMessage($"Cannot Add {_transaction.SelectedLineItem.TransactionQuantity} as Available stock is {TransactionQuantiy.Value.ToString()}");
